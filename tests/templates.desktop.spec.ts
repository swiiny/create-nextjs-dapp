...
const templates = [
	...
	{ name: 'Stylex', path: 'packages/stylex', port: 3004, localOnly: true },
	{ name: 'Tailwind', path: 'packages/tailwind', port: 3005, localOnly: true }
];

const isCIEnv = !!process.env.CI;

templates
	// run localOnly templates only in local environment
	.filter(({ localOnly }) => !isCIEnv || !localOnly)
	.forEach(({ name, path: templatePath, port }) => {
